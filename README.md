...
On by default. It's laptop-only (it needs a lid), the sound plays through your speakers even with the lid shut, and it lands within about 2 seconds of closing. The output volume is briefly raised to 65% for the chime, so it's audible even if the machine is muted or turned down, then restored to its previous level and mute state. Reopening and re-closing in quick succession may skip the chime.
